...
export class Game {
  ...
  async getLatestPublished({ platformId = null }) {
    try {
      const filterPlatform =
        platformId && `filters[platform][id][$eq]=${platformId}`;
      const sort = `sort[0]=publishedAt:desc`;
      const populate = `populate=*`;
      const urlParams = `${sort}&${filterPlatform}&${populate}`;
  ...
  async getBySlug(slug) {
    try {
      ...
      const populate = `populate[0]=wallpaper&populate[1]=cover&populate&populate[2]=platform`;
